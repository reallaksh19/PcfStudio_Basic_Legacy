...
export function initViewerTab() {
    ...
    _dom.generateBtn?.addEventListener('click', () => {
        // Ensure WebGL initializes when generate is actually clicked
        if (_viewMode === '3D') {
            _switchView('3D');
        }
        _runGenerate();
...
function _exportPcfFromTable() {
    ...
    for (const comp of components) {
        ...
        if (comp.centrePoint) {
            ...
            lines.push(`    CENTRE-POINT  ${cp.x.toFixed(4)} ${cp.y.toFixed(4)} ${cp.z.toFixed(4)}`);
        }
...
function _renderTableView() {
    ...
    if (_processedData.components.length === 0 && !_dom.input?.value?.trim()) {
