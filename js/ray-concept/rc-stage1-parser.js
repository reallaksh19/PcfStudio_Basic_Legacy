 ...
 * 100% independent — only imports from rc-* siblings and shared bore converter
 */
...
import { convertBoreValue } from '../services/bore-converter.js';
...
function parseConvertedBore(row, cfg, label = 'BORE') {
  const rawBore = resolveCol(row || {}, COL.bore);
  const rawOd = resolveCol(row || {}, COL.od);
  const converted = convertBoreValue(rawBore, { sourceColumn: label, odFallback: rawOd });
  const fallback = parseUnit(rawBore, cfg);
  return {
    bore: Number.isFinite(converted.boreMm) ? converted.boreMm : fallback,
    rawBore,
    rawOd,
    converted
  };
...
function cleanRefNo(raw) { return raw ? raw.replace(/^=/, '').trim() : ''; }
function derivePipelineRef(row) { return resolveCol(row || {}, COL.pipelineRef) || ''; }
function computeBrlen(bp, cp) { if (!bp || !cp) return 0; const d = vecSub(bp, cp); return vecMag(d); }

const SKEW_GUARD_DEFAULTS = { maxDiagonalGap: 2000, maxPipeRun: 30000, singleAxisSlopeTolerance: 0.01 };
...
function analyzeSkew(ep1, ep2, validationCfg) {
  ...
  const dx = Math.abs(ep2.x - ep1.x), dy = Math.abs(ep2.y - ep1.y), dz = Math.abs(ep2.z - ep1.z);
  ...
  return { len, effAxes, largeSkew, maxDiagonalGap: guard.maxDiagonalGap, maxPipeRun: guard.maxPipeRun };
}

function resolveSkey(type, cfg) { return cfg.skeyMap[type] ?? ''; }

const CSV2D_HEADERS = [
  'CSV SEQ NO', 'Type', 'REF NO.', 'BORE', 'BRANCH BORE',
  'EP1 X', 'EP1 Y', 'EP1 Z', 'EP2 X', 'EP2 Y', 'EP2 Z',
  'CP X', 'CP Y', 'CP Z', 'BP X', 'BP Y', 'BP Z',
  'SUPPORT COOR X', 'SUPPORT COOR Y', 'SUPPORT COOR Z',
  'SUPPORT NAME', 'SUPPORT GUID', 'SKEY',
  ...
  'CA9 (Fluid Den.)', 'CA10 (Hydro Pr.)', 'PIPING CLASS', 'RATING', 'LINENO KEY'
];
...
export function runStage1(rawCsvText, logFn = () => {}, validationCfg = null) {
  ...
  for (const row of rawRows) {
    ...
    if (!groups.has(key)) { groups.set(key, { rawRef, rows: [] }); groupOrder.push(key); }
  ...
  for (const key of groupOrder) {
    ...
    const byPoint = {};
    for (const r of rows) byPoint[resolveCol(r, COL.point)] = r;

    const boreRow = byPoint['1'] ?? byPoint['0'] ?? firstRow;
    const boreInfo = parseConvertedBore(boreRow, cfg, 'BORE');
    const bore = boreInfo.bore;
    ...
    const branchBoreInfo = byPoint['3'] ? parseConvertedBore(byPoint['3'], cfg, 'BRANCH BORE') : null;
    const branchBore = branchBoreInfo ? branchBoreInfo.bore : null;
    ...
    const derivedSupName = canonType === 'SUPPORT' ? deriveSupportName(restrFriction, restrGap, cfg) : '';
    ...
    const ca97 = (canonType !== 'PIPE' && canonType !== 'SUPPORT' && refNo) ? `=${refNo}` : '';
    let supportCoord = null;
    if (canonType === 'SUPPORT') {
      const supportRow = byPoint['0'] ?? byPoint['1'] ?? byPoint['2'] ?? Object.values(byPoint)[0] ?? firstRow;
      if (supportRow) {
        const raw = parseCoord(supportRow, cfg);
        if (!isNaN(raw.x) && !isNaN(raw.y) && !isNaN(raw.z)) supportCoord = raw;
      }
    }
    ...
    const comp = {
      ...
      ca1: resolveCol(firstRow, COL.ca1), ca2: resolveCol(firstRow, COL.ca2), ca3: resolveCol(firstRow, COL.ca3), ca4: resolveCol(firstRow, COL.ca4), ca5: resolveCol(firstRow, COL.ca5),
      ca6: resolveCol(firstRow, COL.ca6), ca7: resolveCol(firstRow, COL.ca7), ca8: resolveCol(firstRow, COL.ca8), ca9: resolveCol(firstRow, COL.ca9), ca10: resolveCol(firstRow, COL.ca10),
      ...
      rawBore: boreInfo.rawBore,
      boreConversion: boreInfo.converted,
      branchBoreConversion: branchBoreInfo?.converted || null
    };

    const skew = canonType === 'PIPE' ? analyzeSkew(ep1, ep2, validationCfg) : null;
    if (skew?.largeSkew) {
      logFn('S1', 'excluded', refNo, { reason: `3D skew ${fmtNum(skew.len, cfg)}mm > ${fmtNum(Math.min(skew.maxDiagonalGap, skew.maxPipeRun), cfg)}mm`, effAxes: skew.effAxes, maxDiagonalGap: skew.maxDiagonalGap, maxPipeRun: skew.maxPipeRun });
      continue;
    }

    components.push(comp);
    logFn('S1', 'component-built', refNo, {
      canonType,
      bore,
      rawBore: boreInfo.rawBore,
      boreConversionStatus: boreInfo.converted.status,
      branchBore,
      branchBoreConversionStatus: branchBoreInfo?.converted?.status || '',
      ep1, ep2, cp, bp
    });
  }
...
function fmtCell(v, cfg) { if (v === null || v === undefined || v === '') return ''; if (typeof v === 'number') return fmtNum(v, cfg); return String(v); }
function coordCell(pt, axis, cfg) { if (!pt) return ''; const val = pt[axis]; return isNaN(val) ? '' : fmtNum(val, cfg); }
...
export function emit2DCSV(components, cfg) {
  ...
  for (const c of components) {
    const la = c.lenAxis || {};
    const cells = [
      c.seqNo, c.type, c.refNo,
      isNaN(c.bore) ? '' : fmtNum(c.bore, cfg),
      (c.branchBore !== '' && !isNaN(c.branchBore)) ? fmtNum(Number(c.branchBore), cfg) : '',
      coordCell(c.ep1, 'x', cfg), coordCell(c.ep1, 'y', cfg), coordCell(c.ep1, 'z', cfg),
      coordCell(c.ep2, 'x', cfg), coordCell(c.ep2, 'y', cfg), coordCell(c.ep2, 'z', cfg),
      coordCell(c.cp, 'x', cfg), coordCell(c.cp, 'y', cfg), coordCell(c.cp, 'z', cfg),
      coordCell(c.bp, 'x', cfg), coordCell(c.bp, 'y', cfg), coordCell(c.bp, 'z', cfg),
      coordCell(c.supportCoor, 'x', cfg), coordCell(c.supportCoor, 'y', cfg), coordCell(c.supportCoor, 'z', cfg),
      c.supportName, c.supportGuid, c.skey,
      la.len1 ?? '', la.axis1 ?? '', la.len2 ?? '', la.axis2 ?? '', la.len3 ?? '', la.axis3 ?? '',
      c.brlen ?? '', c.pipelineRef, c.ca97, c.seqNo,
      c.ca1 ?? '', c.ca2 ?? '', c.ca3 ?? '', c.ca4 ?? '', c.ca5 ?? '', c.ca6 ?? '', c.ca7 ?? '', c.ca8 ?? '', c.ca9 ?? '', c.ca10 ?? '',
      (c.pipingClass || (c.type !== 'SUPPORT' ? (cfg?.defaultPipingClass || '') : '')) ?? '', c.rating ?? '', c.lineNoKey ?? ''
    ];
