  ...
  setExternalDataTable: (rows) => {
      ...
      const components = safeRows.map((row, idx) => {
          ...
          return {
              ...
              ca,
              points: points,
              ep1: (row.type || 'UNKNOWN').toUpperCase() === 'SUPPORT' ? null : row.ep1,
              ep2: (row.type || 'UNKNOWN').toUpperCase() === 'SUPPORT' ? null : row.ep2,
              cp: (row.type || 'UNKNOWN').toUpperCase() === 'SUPPORT' ? null : (row.cp || null),
              bp: (row.type || 'UNKNOWN').toUpperCase() === 'SUPPORT' ? null : (row.bp || null),
              supportCoor: row.supportCoor || ((row.type || 'UNKNOWN').toUpperCase() === 'SUPPORT' ? (row.cp || row.ep1 || row.coOrds) : null),
              attributes: {
